...
بازار تتریِ مارجین‌دار دارند (نه یک لیست ثابت) — به درخواست کاربر، دقیقاً
...
from signal_bot import (  # noqa: E402
...
SOURCE_CONFIG = {
    SOURCE_ST: {
        "margin_usdt": _float_env("BOT2_ST_MARGIN_USDT", "10"),
        "leverage": _float_env("BOT2_ST_LEVERAGE", "3"),
    },
    SOURCE_SMC: {
        "margin_usdt": _float_env("BOT2_SMC_MARGIN_USDT", "10"),
...
def _close_lot(state, spot_symbol, pos, lot_key, price, reason, source_label):
    ...
    if DRY_RUN:
        ...
        notify(
            ...
            f"قیمت خروج≈{fmt_price(price)} {cur} | تغییر قیمت: {pct_sign}{price_pct:.2f}٪ (سود/زیان دقیق رو از پنل تبدیل چک کن)"
        )
...
def manage_open_position(state: dict, position_key: str, spot_symbol: str):
    ...
    trading_bot_v2.py قدیمی، ولی روی داده‌ی واقعی تبدیل نه بایننس.
    """
...
def main():
    ...
    # --- فیلتر پیش‌کشف: مثل بات۱ - نمادهایی که روی بایننس اصلاً وجود ندارن
    # همین‌جا رد می‌شن، نه توی لوپ با پیام خطای تکراری به تلگرام. ---
    binance_to_symbols = {}
    skipped_no_binance = []
